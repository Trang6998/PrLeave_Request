...
namespace CMS.Controllers
{
    ...
    public class UsersController : BaseApiController
    {
        ...
        public async Task<IHttpActionResult> InsertAsync([FromBody]Users users)
        {
            if (users.UserId != 0)
            ...
            using (var db = new ApplicationDbContext())
            {
                ...
                Users userExits = db.Users.SingleOrDefault(x => x.UserName == users.UserName);
                if (userExits == null)
                {
        ...
        public async Task<IHttpActionResult> UpdateAsync(int userId, [FromBody]Users users)
        {
            ...
            using (var db = new ApplicationDbContext())
            {
                Users userExits = db.Users.Find(userId);
        ...
        public async Task<IHttpActionResult> UpdatePasswordAsync(int userId, [FromBody]ChangePassDto changePass)
        {
            ...
            using (var db = new ApplicationDbContext())
            {
                Users userExits = db.Users.SingleOrDefault(x => x.UserName == User.Identity.Name);
                if (userExits == null)
                {
        ...
        public async Task<IHttpActionResult> DeleteAsync(int userId)
        {
            using (var db = new ApplicationDbContext())
            {
                var users = await db.Users.SingleOrDefaultAsync(o => o.UserId == userId);
